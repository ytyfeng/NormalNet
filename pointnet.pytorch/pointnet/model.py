...
class PointNetfeat(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 1024)
        print("x size in pointnetFeat forward2:")
        print(x.size())
        if self.global_feat:
...
class NormalNet(nn.Module):
    def __init__(self, k = 3, global_feat = False, sym_op='max', feature_transform = False, output_dim=3):
    ...
    def forward(self, x):
        ...
        print("x size in normalnet forward:")
        print(x.size())
        return x, trans, trans_feat
        '''
    ...
    def forward(self, x):
        batchsize = x.size()[0]
        n_pts = x.size()[2]
        print("x before forward: " + str(x.size()))
        x, trans, trans_feat = self.feat(x)
        x = F.relu(self.bn1(self.conv1(x)))
        x = F.relu(self.bn2(self.conv2(x)))
        x = F.relu(self.bn3(self.conv3(x)))
...
class PCPNet(nn.Module):
    def __init__(self, num_points=2500, output_dim=3, use_point_stn=True, use_feat_stn=True, sym_op='max', get_pointfvals=False, point_tuple=1):
        super(PCPNet, self).__init__()
        self.num_points = num_points

        self.feat = PointNetfeat2(
